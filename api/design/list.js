...
export default async function handler(req, res){
  ...
  try{
    // 兼容：优先从 design_assets 取最新附件，其次回退到 files 表
    const r = await q(`
      SELECT dp.id, dp.description, dp.file_id, dp.external_url, dp.created_at,
      ...
      LEFT JOIN files f ON f.id = dp.file_id
    ...
    const rows = r.rows.map(x => ({
      ...
      file_url: x.asset_url ? x.asset_url : (x.file_id ? `/api/files/${x.file_id}` : null),
    }));

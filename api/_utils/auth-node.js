...
export function requireAuth(req){ const a = req.headers.authorization || ''; const t = a.startsWith('Bearer ') ? a.slice(7) : ''; if(!t) throw new Error('missing token'); const d = jwt.verify(t, process.env.JWT_SECRET || 'dev'); if(!d?.userId) throw new Error('invalid token'); return d.userId; }
